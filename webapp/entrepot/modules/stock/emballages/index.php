                  ...
                  <?php $total = 0; foreach ($emballages as $key => $emballage) {
                    ...
                    <div class="col-sm-4 col-md-3 col-lg-2 border-left border-bottom">
                        <div class="p-xs">
                            ...
                            <h6 class="m-xs gras <?= ($stock > $params->ruptureStock)?"":"clignote" ?>"><?= round($stock, 2) ?> <?= $emballage->abbr ?></h6>
                            ...
                            while ($index <= $date2) { ?>
                                <tr>
                                    ...
                                    <?php foreach ($emballages as $key => $emballage) {
                                        ...
                                        data-html="true"
                                        data-content="
                                        <span>Appro du jour : <b><?= round($appro, 2) ?> <?= $emballage->abbr ?></b></span><br>
                                        <span>Conso du jour : <b><?= round($conso, 2) ?> <?= $emballage->abbr ?></b></span><br>
                                        <span>Perte : <b><?= round($perte, 2) ?> <?= $emballage->abbr ?></b></span>
                                        <hr style='margin:1.5%'>
                                        <span>En stock à ce jour : <b><?= round($stock, 2) ?> <?= $emballage->abbr ?></b></span><br> <span>">
                                            <?= round($stock, 2) ?> <?= $emballage->abbr ?>
                                        </td>
                                    <?php } ?>
                                </tr>
                                <?php
