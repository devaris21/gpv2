...
<body class="fixed-sidebar">
    ...
    <div id="wrapper">
                            ...
                            <tbody>
                                <?php foreach (Home\PARFUM::findBy(["isActive ="=>Home\TABLE::OUI]) as $key => $parfum) {
                                    ...
                                    if ($qua > 0) { ?>
                                        <tr>
                                            <td>
                                                <button data-toggle="modal" data-target="#modal-conditionnement-<?= $type->getId() ?>-<?= $parfum->getId() ?>"  class="btn btn-primary btn-xs pull-right"><?= start0($qua) ?> <?= $type->abbr  ?></button> <?= $parfum->name(); ?>
                                            </td>
                                        </tr>
                                    <?php } 
                                }  ?>
        ...
        <?php foreach (Home\TYPEPRODUIT::findBy(["isActive ="=>Home\TABLE::OUI]) as $key => $type) {
            foreach (Home\PARFUM::findBy(["isActive ="=>Home\TABLE::OUI]) as $key => $parfum) {
                $qua = Home\PRODUCTION::enStock(Home\PARAMS::DATE_DEFAULT, dateAjoute(1), $type->id, $parfum->id, $entrepot->id);
